	...
	function trx_addons_cpt_agents_init() {
		...
		$TRX_ADDONS_STORAGE['meta_box_'.TRX_ADDONS_CPT_AGENTS_PT] = array(
			...
			"socials_info" => array(
				...
				"desc" => wp_kses_data( __("Specify URLs of agent's profiles in the popular social networks", 'trx_addons') ),
				"type" => "info"
			),
			"facebook" => array(
			...
			"gplus" => array(
				"title" => esc_html__("Google Plus", 'trx_addons'),
				"desc" => '',
				"class" => "trx_addons_column-1_3",
				"std" => "",
				"type" => "text"
			...
			"youtube" => array(
				"title" => esc_html__("Youtube", 'trx_addons'),
				"desc" => '',
				"class" => "trx_addons_column-1_3",
				"std" => "",
				"type" => "text"
			...
			"vimeo" => array(
				"title" => esc_html__("Vimeo", 'trx_addons'),
				"desc" => '',
				"class" => "trx_addons_column-1_3",
				"std" => "",
				"type" => "text"
...
if ( !function_exists( 'trx_addons_properties_get_agent_socials' ) ) {
	function trx_addons_properties_get_agent_socials($meta) {
		$icons = array();
		if (!empty($meta['email']))		$icons[] = array('name'	=> 'mail', 'url' => sprintf('mailto:%s', antispambot($meta['email'])));
		if (!empty($meta['skype']))		$icons[] = array('name'	=> 'skype', 'url' => sprintf('skype:%s', $meta['skype']));
		if (!empty($meta['facebook']))	$icons[] = array('name'	=> 'facebook', 'url' => $meta['facebook']);
		if (!empty($meta['twitter']))	$icons[] = array('name'	=> 'twitter', 'url' => $meta['twitter']);
		if (!empty($meta['instagram']))	$icons[] = array('name'	=> 'instagram', 'url' => $meta['instagram']);
		if (!empty($meta['gplus']))		$icons[] = array('name'	=> 'gplus', 'url' => $meta['gplus']);
		if (!empty($meta['youtube']))	$icons[] = array('name'	=> 'youtube', 'url' => $meta['youtube']);
		if (!empty($meta['vimeo']))		$icons[] = array('name'	=> 'vimeo', 'url' => $meta['vimeo']);
		return $icons;
	}
}
