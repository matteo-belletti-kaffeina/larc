...
jQuery(document).ready(function() {
	...
	function trx_addons_options_init_fields(e, container) {
		...
		if (jQuery.ui.sortable) {
			container.find('.trx_addons_options .trx_addons_options_item_choises.trx_addons_options_sortable:not(.inited)')
				.addClass('inited')
				.each(function() {
		...
		// Init checklist
		container.find('.trx_addons_options .trx_addons_options_item_choises:not(.inited)').addClass('inited')
		...
		if (jQuery.fn && jQuery.fn.select2) 
			container.find('.trx_addons_options .trx_addons_options_item_select2 select:not(.inited)').addClass('inited').select2();
	
		// Init datepicker
		if (jQuery.ui.datepicker) {
			container.find('.trx_addons_options .trx_addons_options_item_date input[type="text"]:not(.inited)').addClass('inited')
				.each(function () {
		...
		// Init masked input
		container.find('.trx_addons_options .trx_addons_options_item input[data-mask]:not(.inited)').addClass('inited')
			.each(function () {
		...
		container.find('.trx_addons_options .trx_addons_options_group:not(.inited)').addClass('inited').each(function() {
			jQuery(this)
				// Button 'Clone'
		...
		function trx_addons_options_clone(obj) {
			...
			jQuery(document).trigger('action.init_hidden_elements', [clone]);
		}
	}
