...
$possible_plugin_locations = array(
	...
	'/var/www/vhosts/larc.it/httpdocs/wp-content/plugins/wp-optimize-premium/cache',
);
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', '/var/www/vhosts/larc.it/httpdocs/wp-content/plugins/wp-optimize-premium/cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-larc.it.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-larc.it.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-larc.it.php');
}
