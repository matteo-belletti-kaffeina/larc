/* global luxmed_color_schemes, luxmed_dependencies, Color */
...
( function( api ) {
	...
	for (var i in luxmed_color_schemes) {
		cssTemplate[i] = wp.template( 'luxmed-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'luxmed-fonts' );
	
	// Set initial state of controls
	api.bind('ready', function() {
		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+luxmed_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(luxmed_customizer_vars['msg_reset_confirm'])) {
		...
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', luxmed_customizer_update_load_fonts);		
		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', luxmed_customizer_actions);
		// Check dependencies in the each section
		jQuery('#customize-theme-controls .control-section').each(function () {
			luxmed_customizer_check_dependencies(jQuery(this));
		});
		// Refresh color controls
		luxmed_customizer_change_color_scheme(api('color_scheme_editor')());
	});
	
	// On change any control - check for dependencies
	api.bind('change', function(obj) {
		if (obj.id == 'scheme_storage') return;
		luxmed_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		luxmed_customizer_refresh_preview(obj);
	});

	// Check for dependencies
	function luxmed_customizer_check_dependencies(cont) {
		cont.find('.customize-control').each(function() {
			...
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('luxmed_options_no_use');
				} else {
					jQuery(this).hide().addClass('luxmed_options_no_use');
				}
			}
		});
	}
	...
	function luxmed_customizer_refresh_preview(obj) {
		...
		if (id == 'color_scheme_editor') {
			luxmed_customizer_change_color_scheme(val);

		} else if (updateCSS) {
			// Check colors in scheme
			var simple = api('color_settings')()=='simple';
			for (opt in luxmed_color_schemes['default'].colors) {
				if (opt == id) {
					// Store new value in the color table
					luxmed_customizer_update_color_scheme(opt, val);
					...
					if (simple) {
						if (id == 'text_link') {
							...
							luxmed_customizer_update_color_scheme('alter_link', val);
						} else if (id == 'text_hover') {
							api('alter_hover').set( val );
							api.control( 'alter_hover' ).container.find( '.color-picker-hex' )
								.data( 'data-default-color', val )
								.wpColorPicker( 'defaultColor', val );
							luxmed_customizer_update_color_scheme('alter_hover', val);
						}
					}
					processed = true;
					break;
			...
			// Check theme fonts
			if (!processed) {
				for (opt in luxmed_theme_fonts) {
					for (rule in luxmed_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value in the color table
							luxmed_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed) luxmed_customizer_update_css();
		}
	...
	// Actions buttons
	function luxmed_customizer_actions(e) {
	...
	// Store new value in the color table
	function luxmed_customizer_update_color_scheme(opt, value) {
		luxmed_color_schemes[api('color_scheme_editor')()].colors[opt] = value;
		api('scheme_storage').set(luxmed_serialize(luxmed_color_schemes))
	}
	
	// Store new value in the theme fonts
	function luxmed_customizer_update_theme_fonts(opt, rule, value) {
		luxmed_theme_fonts[opt][rule] = value;
	}
	
	// Change theme fonts options if load fonts is changed
	function luxmed_customizer_update_load_fonts() {
		var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
		updateCSS = false;
		for (i=1; i<=luxmed_customizer_vars['max_load_fonts']; i++) {
			name = api('load_fonts-'+i+'-name')();
			if (name == '') continue;
			family = api('load_fonts-'+i+'-family')();
			opt_list.push([name, family]);
		}
		for (tag in luxmed_theme_fonts) {
	...
	function luxmed_customizer_change_color_scheme(value) {
		updateCSS = false;
		for (var opt in luxmed_color_schemes[value].colors) {
			if (api(opt) == undefined) continue;
			api( opt ).set( luxmed_color_schemes[value].colors[opt] );
			api.control( opt ).container.find( '.color-picker-hex' )
				.data( 'data-default-color', luxmed_color_schemes[value].colors[opt] )
				.wpColorPicker( 'defaultColor', luxmed_color_schemes[value].colors[opt] );
		}
		updateCSS = true;
		luxmed_customizer_update_css();
	}
	
	// Generate the CSS for the current Color Scheme and send it to the preview window
	function luxmed_customizer_update_css() {
		...
		var fonts = luxmed_customizer_add_theme_fonts(luxmed_theme_fonts);

		// Make styles and add into css
		css += luxmed_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));

		// Add colors
		for (var scheme in luxmed_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in luxmed_color_schemes[scheme].colors) {
				colors[i] = luxmed_color_schemes[scheme].colors[i];
			}
			
			// Make theme specific colors and tints
			colors = luxmed_customizer_add_theme_colors(colors);
	...
	function luxmed_customizer_add_theme_colors(colors) {
		...
		colors.bg_color_01 = Color( colors.bg_color ).toCSS( 'rgba', 0.1 );
		...
		colors.text_light_05 = Color( colors.text_light ).toCSS( 'rgba', 0.5 );
		...
		colors.accent2_blend = luxmed_hsb2hex(luxmed_hex2hsb( colors.accent2_hover, 0, 0, -15 ));
		return colors;
	}
	...
	function luxmed_customizer_add_theme_fonts(fonts) {
		...
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' && fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + luxmed_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' && fonts[tag]['margin-top'] != '' && fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + luxmed_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' && fonts[tag]['margin-bottom'] != '' && fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + luxmed_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	// Add ed to css value
	function luxmed_customizer_prepare_css_value(val) {
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
		}
	...
	function luxmed_customizer_prepare_html_value(val) {
