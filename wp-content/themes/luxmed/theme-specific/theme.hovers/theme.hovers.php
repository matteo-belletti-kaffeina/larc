...
if (!function_exists('luxmed_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'luxmed_hovers_theme_setup3', 3 );
	function luxmed_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		luxmed_storage_set_array_before('options', 'sidebar_widgets', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'luxmed'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'luxmed') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'luxmed'),
					'slide_left'	=> esc_html__('Slide from Left',	'luxmed'),
					'slide_right'	=> esc_html__('Slide from Right',	'luxmed'),
					'slide_top'		=> esc_html__('Slide from Top',		'luxmed'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'luxmed'),
				),
				"type" => "hidden" //select
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'luxmed'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'luxmed') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'luxmed'),
					'icon'	=> esc_html__('Icon',	'luxmed'),
					'icons'	=> esc_html__('Icons',	'luxmed'),
					'zoom'	=> esc_html__('Zoom',	'luxmed'),
					'fade'	=> esc_html__('Fade',	'luxmed'),
					'slide'	=> esc_html__('Slide',	'luxmed'),
					'pull'	=> esc_html__('Pull',	'luxmed'),
					'border'=> esc_html__('Border',	'luxmed')
				),
				"type" => "hidden" //select
			) )
		);
...
if (!function_exists('luxmed_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'luxmed_hovers_theme_setup9', 9 );
	function luxmed_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'luxmed_hovers_frontend_scripts', 1010 );
		add_filter( 'luxmed_filter_localize_script','luxmed_hovers_localize_script' );
		add_filter( 'luxmed_filter_merge_scripts',	'luxmed_hovers_merge_scripts' );
		add_filter( 'luxmed_filter_merge_styles',	'luxmed_hovers_merge_styles' );
		add_filter( 'luxmed_filter_get_css', 		'luxmed_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'luxmed_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'luxmed_hovers_frontend_scripts', 1010 );
	function luxmed_hovers_frontend_scripts() {
		if ( luxmed_is_on(luxmed_get_theme_option('debug_mode')) && luxmed_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'luxmed-hovers', luxmed_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( luxmed_is_on(luxmed_get_theme_option('debug_mode')) && luxmed_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'luxmed-hovers',  luxmed_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('luxmed_hovers_merge_scripts')) {
	//Handler of the add_filter( 'luxmed_filter_merge_scripts', 'luxmed_hovers_merge_scripts' );
	function luxmed_hovers_merge_scripts($list) {
...
if (!function_exists('luxmed_hovers_merge_styles')) {
	//Handler of the add_filter( 'luxmed_filter_merge_styles', 'luxmed_hovers_merge_styles' );
	function luxmed_hovers_merge_styles($list) {
...
if (!function_exists('luxmed_hovers_localize_script')) {
	//Handler of the add_filter( 'luxmed_filter_localize_script','luxmed_hovers_localize_script' );
	function luxmed_hovers_localize_script($arr) {
		$arr['button_hover'] = luxmed_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('luxmed_hovers_add_icons') ) {
	function luxmed_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'luxmed') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink());; ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') esc_html_e('Details', 'luxmed');
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'luxmed_hovers_get_css' ) ) {
	//Handler of the add_filter( 'luxmed_filter_get_css', 'luxmed_hovers_get_css', 10, 4 );
	function luxmed_hovers_get_css($css, $colors, $fonts, $scheme='') {
...
body[class*="blog_style_portfolio_"] .post_featured.hover_dots .icons:hover:before {
...
.post_featured.hover_dots .icons:hover span {
	background-color: {$colors['text_link']};
}
